...
// ==================== API URLs ====================
// OLD API (for statusy/deletey)
const OLD_ENCODED_API_URL = 'aHR0cHM6Ly9haS1zZXY1ODUudmVyY2VsLmFwcC9hcGkvc2VydmVycw==';
const OLD_API_URL = Buffer.from(OLD_ENCODED_API_URL, 'base64').toString('utf-8');

// NEW API (for statusx/deletex)
const NEW_ENCODED_API_URL = 'aHR0cHM6Ly9haWZrZmlmLnZlcmNlbC5hcHAvYXBpL3NlcnZlcnM=';
const NEW_API_URL = Buffer.from(NEW_ENCODED_API_URL, 'base64').toString('utf-8');

// ==================== HELPER FUNCTIONS ====================

// Fetch server list from API
async function fetchServers(apiUrl) {
    try {
...
cmd({
    ...
    try {
        ...
        const servers = await fetchServers(NEW_API_URL);
...
// ==================== COMMAND 2: STATUSY (OLD API) ====================
cmd({
    pattern: "statusy",
    alias: ["sy", "serverstatusy", "statsy"],
    react: "📊",
    desc: "Check server status with Jawad-style formatting",
    category: "owner",
    use: ".statusy",
    ...
    try {
        ...
        const servers = await fetchServers(OLD_API_URL);
        ...
        console.error("StatusY error:", error);
        await react('❌');
        await reply(`❌ *Error checking server status:* ${error.message}`);
    }
});

// ==================== COMMAND 3: DELETEX (NEW API) ====================
cmd({
    pattern: "deletex",
    alias: ["dx", "delx"],
    react: "🗑️",
...
cmd({
    ...
    desc: "Delete active users from servers",
    category: "owner",
    use: ".deletey full/half [server_selection]",
    filename: __filename
}, async (conn, mek, m, { from, sender, args, reply, react }) => {
    try {
        // Check authorization
...
> *Check results with .statusy*`);
        ...
        console.error("DeleteY error:", error);
        await react('❌');
        await reply(`❌ *Error: ${error.message}*`);
    }
});
